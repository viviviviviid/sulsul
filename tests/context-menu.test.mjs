...
test('context menus install once, rebuild without duplicates, and survive worker restarts',async()=>{
  ...
  assert.equal(w.menus.size,7);
  assert.equal(w.menus.get('sulsul').title,'술술');
  ...
  assert.equal(restarted.menus.size,7);
  await restarted.callbacks.startup();
  assert.equal(restarted.menus.size,7);
});

test('menu actions target the clicked webpage main frame; settings needs no page injection',async()=>{
  const w=worker(),tab={id:0,url:'https://example.com/page'};
  for(const type of ['sulsul-start','sulsul-stop','sulsul-restore','sulsul-end']){
