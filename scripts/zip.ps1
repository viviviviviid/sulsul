param([Parameter(Mandatory=$true)][string]$Source,[Parameter(Mandatory=$true)][string]$Destination,[switch]$ContentsOnly)
$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.IO.Compression.FileSystem
Add-Type -AssemblyName System.IO.Compression
$parent = [IO.Path]::GetDirectoryName([IO.Path]::GetFullPath($Source))
if ($ContentsOnly) { $parent = [IO.Path]::GetFullPath($Source) }
