...
for(const name of source?sourceFiles:runtimeFiles){
  const target=path.join(folder,name);fs.mkdirSync(path.dirname(target),{recursive:true});
  let text=fs.readFileSync(path.join(root,name),'utf8');
  if(/\.(ps1|txt)$/.test(name))text='\ufeff'+text.replace(/^\uFEFF/,'').replace(/\r?\n/g,'\r\n');
  fs.writeFileSync(target,text);
}
const zip=path.join(dist,source?`Sulsul-Source-${version}.zip`:`Sulsul-Windows-${version}.zip`);
const shell=process.platform==='win32'?'powershell.exe':'pwsh';
const temporaryZip=path.join(stage,'archive.zip');
const result=spawnSync(shell,['-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',path.join(root,'scripts/zip.ps1'),'-Source',folder,'-Destination',temporaryZip],{stdio:'inherit'});
