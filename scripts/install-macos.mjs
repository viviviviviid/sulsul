...
for (const arg of args) if (!['--without-antigravity','--skip-download','--uninstall'].includes(arg)) throw new Error('알 수 없는 옵션: '+arg);
...
const installed = path.join(os.homedir(),'Library','Application Support','Sulsul','runtime');
const installedHost = path.join(installed,'host');
const registry = path.join(os.homedir(),'Library','Application Support','Google','Chrome','NativeMessagingHosts','com.sulsul.gemini.json');
if (args.has('--uninstall')) {
  ...
  if (!args.has('--without-antigravity') && !args.has('--skip-download')) {
    ...
    const runtime = path.join(root,'data','sulsul-agy');
    ...
    try {
      ...
      fs.writeFileSync(path.join(root,'data','agy-release.json'),JSON.stringify(release,null,2)+'\n');
  ...
  fs.copyFileSync(path.join(root,'extension','manifest.json'),path.join(installed,'extension','manifest.json'));
  for (const name of ['core.mjs','runner.mjs','host.mjs','setup-runtime.mjs','translation.schema.json','login.mjs','provider-settings.mjs','providers.mjs','secret-store-macos']) {
  ...
  for (const name of ['sulsul-agy/agy','agy-release.json']) {
    ...
    if (fs.existsSync(source)) fs.copyFileSync(source,path.join(installedData,name));
  }
  ...
  execFileSync(process.execPath,[path.join(installedHost,'setup-runtime.mjs')],{stdio:'inherit'});
