...
const config = { node: process.execPath, cli:fs.existsSync(path.join(runtime,'agy.exe')) ? path.join(runtime,'agy.exe') : previous.cli || path.join(runtime,'agy.exe'), cliVersion:release.version || previous.cliVersion || null, model:previous.model || 'gemini-3.8-flash-low', profile, workspace, extensionId, providerSettings:previous.providerSettings || path.join(profile,'providers.json'), settings:path.join(profile,'.gemini','antigravity-cli','settings.json'), schema:path.join(host,'translation.schema.json') };
...
write(path.join(host,'com.sulsul.gemini.json'),{name:'com.sulsul.gemini',description:'Sulsul AI translation host',path:path.join(host,'sulsul-host.exe'),type:'stdio',allowed_origins:[`chrome-extension://${extensionId}/`]});
