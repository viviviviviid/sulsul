...
import {PROMPT_VERSION,validateTargetLanguage,TARGET_LANGUAGES} from './core.mjs';

export const PROVIDERS=Object.freeze({codex:{label:'ChatGPT',model:'default',fast:false,targetLanguage:'ko',key:false,
...
export class ProviderSettings{
  ...
  read(){
    ...
    try{
      ...
      return {selected:'codex',revision:'chatgpt-only:'+state.revision,providers:{codex:{model:state.providers.codex?.model||'default',targetLanguage:validateTargetLanguage(state.providers.codex?.targetLanguage??'ko'),fast:state.providers.codex?.fast===true}}};
    }catch{throw new Error('설정을 읽지 못했습니다. 설치 안내의 설정 복구 방법을 확인해 주세요.');}
  }
  options(state){return {model:validateModel(state.providers.codex?.model||'default'),targetLanguage:validateTargetLanguage(state.providers.codex?.targetLanguage??'ko'),fast:state.providers.codex?.fast===true,hasKey:false};}
  public(state=this.read()){
    ...
    const cacheScope=createHash('sha256').update(JSON.stringify([PROMPT_VERSION,'chatgpt-block-cache',option.model,option.fast,option.targetLanguage])).digest('hex');
    return {selected:'codex',providers:{codex:option},scope,cacheScope,targetLanguages:TARGET_LANGUAGES};
  }
  ...
  save(data){
    const task=this.writes.catch(()=>{}).then(()=>{
      ...
      const state={selected:'codex',revision:randomUUID(),providers:{codex:{model,targetLanguage:validateTargetLanguage(data.targetLanguage??this.read().providers.codex?.targetLanguage??'ko'),fast:data.fast===true}}};
