...
async function handle(message) {
  ...
  try {
    ...
    if (message.type === 'settings-get') return send({ id, ok:true, result:settings.public() });
    if (message.type === 'settings-save') {
      if (active) throw new Error('번역을 중지한 뒤 설정을 저장해 주세요.');
      const task = { id, controller:new AbortController() }; active = task;
    ...
    if (message.type === 'cancel') { active?.controller.abort(); return send({ id, ok: true, result: {} }); }
    if (message.type === 'login') {
      if (active) throw new Error('번역을 중지한 뒤 계정을 연결해 주세요.');
      if (settings.read().selected !== 'antigravity') throw new Error('Google 로그인은 Antigravity를 선택했을 때 사용할 수 있습니다.');
      if (!router.health().installed) throw new Error('install.cmd를 실행하고 Antigravity 설치를 선택해 주세요.');
      const ps = path.join(process.env.SystemRoot, 'System32','WindowsPowerShell','v1.0','powershell.exe');
      // Fixed script and arguments, never browser-provided command text.
      const child = spawn(ps, ['-NoProfile','-ExecutionPolicy','Bypass','-File',path.join(here,'open-login.ps1')], { windowsHide: true, detached: true, stdio: 'ignore', shell: false });
      child.on('error', () => {});
      child.unref();
    ...
    if (active) throw new Error('다른 번역이 진행 중입니다. 잠시 후 다시 시도해 주세요.');
    ...
    try {
      ...
      send({id,ok:true,result:testing ? {message:'연결 성공 · '+result.blocks[0].parts[0].text} : result});
    }
    finally { if (active === task) active = null; }
  } catch (e) { send({ id, ok: false, error: e.message }); }
}
process.stdin.on('data', createDecoder(handle, () => process.exit(1)));
process.stdin.on('end', () => { active?.controller.abort(); setTimeout(() => process.exit(0), 200).unref(); });
process.stdout.on('error', () => { active?.controller.abort(); process.exit(1); });
